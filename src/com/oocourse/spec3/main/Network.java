...
public interface Network {
    ...
    //@ result == (\exists int i; 0 <= i && i < emojiIdList.length; emojiIdList[i] == id);
    ...
    /*@ public normal_behavior
      ...
      @          (\old(messages[i]) instanceof EmojiMessage &&
      @           !containsEmojiId(\old(((EmojiMessage)messages[i]).getEmojiId())) ==>
      @           (\forall int j; 0 <= j && j < messages.length; messages[j] != \old(messages[i]))));
      @ ensures (\forall int i; 0 <= i && i < \old(messages.length);
      @          (!(\old(messages[i]) instanceof EmojiMessage) ==>
      @           (\exists int j; 0 <= j && j < messages.length; messages[j] == \old(messages[i]))));
    ...
    /*@ public normal_behavior
      ...
      @*/
    public /*@pure@*/ int sendIndirectMessage(int id) throws
